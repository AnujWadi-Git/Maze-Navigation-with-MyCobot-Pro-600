...
def save_coordinates_to_csv(coordinates, filename='E:/ras/project/project/TEST_files/Final_files/mazexy.csv'):
...
def main():
    # Load the maze image
    img_name = "maze5.png"  # Replace with your maze image
    ...
    robot_size =  20# Size of the robot in pixels change to 70 if end point isnt detected
    ...
    coordinates = [(point[0], point[1]) for point in aligned_points]
    ...
    plt.show()
    # Dilate the walls to account for the robot's size
    kernel_size = int(np.ceil(robot_size / 2))
    ...
    # Convert the image for pathfinding (1s for free space, 0s for walls)
    maze_map = (dilated_img == 255).astype(np.uint8)

    # Detect entrance and exit points
    entrance, exit_point = detect_entrance_exit(dilated_img)
    print(f"Entrance: {entrance}, Exit: {exit_point}")

    # Find the solution path
    solution_path = find_solution_path(maze_map, entrance, exit_point)
    print(f"Initial Solution Path: {solution_path}")

    # Adjust points to center and refine path
    centered_path = adjust_points_to_center(maze_map, solution_path)
    refined_path = refine_path(centered_path)

    # Save path instructions
    instructions = save_path_instructions(refined_path, "final", robot_width=robot_size)
    print(f"Path Instructions: {instructions}")
